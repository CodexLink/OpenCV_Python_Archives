...
from typing import Final # Type Hinting, for Strict Return Hinting Process for Standardization.
...
class AssignmentHandler(object):
    def __init__(self, stored_filename: str, stored_filename_ext: str) -> None:
        ...
        self.stored_image_name : Final = "test" if not stored_filename else stored_filename
        self.stored_image_ext : Final = "jpg" if not stored_filename_ext else stored_filename_ext
        ...
        assert stored_filename_ext in self.acceptable_image_exts, "Extension supplied is not allowed! Use (%s, %s, %s) files instead." % (self.acceptable_image_exts)

        # * Contraint | Check for `stored_filename` should be truthy to type(str).
        assert isinstance(stored_filename_ext, str), "Filename should be or atleast contain a string() and not purely made of %s!" % (str, type(stored_filename_ext))
        ...
        # File Path Handlers
        self.CURRENT_PATH = getcwd() # Initially None, but will be constant virtually after some parts of runtime.
        # * self.CURRENT_PATH_IMG_FILE is just a directory form of the target image.
        self.CURRENT_PATH_IMG_FILE = self.CURRENT_PATH + "/" + self.stored_image_name + "." + self.stored_image_ext
    ...
    def display_options(self: object) -> bool:
        ...
        print("Script Has The Image? : %s" % ("Yes, Can Override the Stored Image." if self.inserted_file_is_valid else "No, Please Input an Image Path To Store."), end="\n\n")
    ...
    def input_or_overwrite_image(self: object) -> bool:
        while 1:
            file_buffer_path = str(input("\nInput your File Path with Extensions to Store, type 0 to cancel |> "))

            if file_buffer_path == '0':
                break

            else:
                is_file = path.isfile(file_buffer_path) and file_buffer_path.endswith(self.acceptable_image_exts)
                if is_file:
                    # Copy the Image from src to this script.
                    file_buffer_path = copy_file(file_buffer_path, self.CURRENT_PATH_IMG_FILE)
                    return True
    ...
    def display_image(self: object) -> bool:
        if self.inserted_file_is_valid:
            ...
            imshow("Image Display for %s" % (self.stored_image_name + "." + self.stored_image_ext), image_disp_buf)
            ...
            print("\nFile %s does not exists! Please input an image first!" % (self.CURRENT_PATH_IMG_FILE))
            sleep(1)
            return True
